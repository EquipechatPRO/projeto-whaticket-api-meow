...
const MessagesList = ({
  ...
  const handleFetchHistory = async () => {
    ...
    if (!whatsappId || String(whatsappId) === "null") return;

    setFetchingHistory(true);
    try {
      await api.post(`/messages/${ticketId}/fetch-history`, { count: 50 });
      // Mensagens chegarão via socket/evento messaging-history.set do Baileys
      setTimeout(() => {
        setPageNumber(1);
...
>
  {!hasMore && messagesList.length > 0 && channel === "whatsapp" && whatsappId && String(whatsappId) !== "null" && (ticketStatus === "open" || ticketStatus === "pending") && (
    <div style={{ display: "flex", justifyContent: "center", padding: "8px 0" }}>
      <Button
        ...
        startIcon={fetchingHistory ? <CircularProgress size={14} /> : null}
      >
        {fetchingHistory ? "Buscando histórico..." : "📥 Carregar mensagens antigas do WhatsApp"}
      </Button>
    </div>
  )}
