...
// ✅ Cache de CompaniesSettings por companyId (TTL 5 min)
// Evita query ao banco em CADA mensagem recebida
const settingsMemCache = new Map<number, { data: any; ts: number }>();
const SETTINGS_MEM_TTL = 5 * 60 * 1000;

const getCachedCompanySettings = async (companyId: number) => {
  const cached = settingsMemCache.get(companyId);
  if (cached && Date.now() - cached.ts < SETTINGS_MEM_TTL) return cached.data;
  const settings = await CompaniesSettings.findOne({ where: { companyId } });
  if (settings) settingsMemCache.set(companyId, { data: settings, ts: Date.now() });
  return settings;
};
