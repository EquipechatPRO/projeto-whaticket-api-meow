...
import {
  ...
  GitBranch, FileInput, StopCircle, UserCheck, Timer,
} from "lucide-react";
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border bg-orange-500/10 rounded-t-xl">
        ...
        <span className="text-xs font-bold text-orange-700 dark:text-orange-300">{d.label || "Transferir"}</span>
      </div>
      <div className="px-3 py-2 space-y-1">
        <p className="text-[11px] text-muted-foreground">
          {d.transferTo === "queue" ? `Fila → ${d.transferQueueId || "..."}` : "Próximo atendente disponível"}
        </p>
        {d.transferMessage && (
          <p className="text-[10px] text-muted-foreground italic line-clamp-2">"{d.transferMessage}"</p>
        )}
      </div>
      <Handle type="source" position={Position.Bottom} className="!w-3 !h-3 !bg-primary !border-2 !border-card" />
    </div>
...
export function WaitNode({ data, selected }: NodeProps) {
  const d = data as unknown as FlowNodeData;
  const unitLabel = d.waitUnit === "hours" ? "h" : d.waitUnit === "minutes" ? "min" : "s";
  return (
    <div className={cn(
      "w-60 bg-card border-2 rounded-xl shadow-md transition-all",
      selected ? "border-primary ring-2 ring-primary/20 shadow-lg" : "border-border"
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border bg-rose-500/10 rounded-t-xl">
        ...
        <span className="text-xs font-bold text-rose-700 dark:text-rose-300">{d.label || "Espera"}</span>
      </div>
      <div className="px-3 py-2 space-y-1">
        <p className="text-[11px] text-muted-foreground">
          Timeout: <span className="font-semibold text-foreground">{d.waitTimeout || "?"}{unitLabel}</span>
        </p>
        <p className="text-[10px] text-muted-foreground">
          Ação: {d.waitTimeoutAction === "end" ? "Encerrar" : d.waitTimeoutAction === "transfer" ? "Transferir" : "Enviar mensagem"}
        </p>
      </div>
      <Handle type="source" position={Position.Bottom} className="!w-3 !h-3 !bg-primary !border-2 !border-card" />
      <Handle type="source" position={Position.Bottom} id="timeout" className="!w-2.5 !h-2.5 !bg-rose-500 !border-2 !border-card !left-[75%]" />
    </div>
  );
}
